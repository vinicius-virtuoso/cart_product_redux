...
export const Wrapper = () => {
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "w-full md:w-1/5 md:border-l md:border-zinc-900 h-screen absolute top-0 z-50 bg-zinc-950 transition-all duration-300",
          {
      ...
      >
        <div className="relative h-full">
          <div className="flex justify-between border-b border-zinc-900 p-5 md:py-9 text-zinc-200">
            <h2>
              Seu carrinho tem{" "}
              <strong>
            ...
            <button onClickCapture={() => dispatch(toggleMenu())}>
              <X />
            </button>
          </div>

          <div className="pb-4 pt-3 px-5 space-y-3 md:space-y-5 overflow-auto h-2/3">
            {products.map((product) => (
              <CardProduct key={product.id} product={product} />
            ))}
          </div>

          <div className="absolute bottom-0 w-full h-48 border-t border-zinc-900 shadow-xl">
            <CartFooter />
          </div>
        </div>
      </div>
